...
{
    public partial class MudAdvancedAutocomplete<T> : MudBaseInput<T>, IDisposable
    {
        ...
        public MudAdvancedAutocomplete()
        {
            ...
            switch (args.Key)
            {
                ...
                case "NumpadEnter":
                    ...
                    if (_selectedListItemIndex >= 0 && _selectedListItemIndex < _items.Length &&
                        _items[_selectedListItemIndex].ToString() == Text)
                    {
                        OnValueChanged.InvokeAsync(_items[_selectedListItemIndex]).AndForget();
                        break;
                    }
