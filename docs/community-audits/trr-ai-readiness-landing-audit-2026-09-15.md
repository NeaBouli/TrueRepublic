...
TrueRepublic's public surface is the smallest of the four projects in this audit series — one static page + status.json — and correspondingly has a bounded AI-readiness gap. The content that *is* there is honest and CI-pinned, which is more than the IFR, Ekklesia, Stealth or Prometheus surfaces could claim across the board (fabricated dashboard, GA contradictions, stale wikis elsewhere). Adding OG/Twitter, canonical, and JSON-LD metadata to `docs/index.html` addresses TRR-38; adding `sitemap.xml`, `robots.txt`, and `llms.txt` addresses TRR-39. The mobile navigation and accessibility work in TRR-40 and the design-system cleanup in TRR-41 remain separate open items, so those additions do not close the entire report.
