...
2. [Introduction](#2-introduction)
   - 2.1 [The 3-Pillars of TrueRepublic](#21-the-3-pillars-of-truerepublic-encouraged-participation-self-organization-data-security)
3. [The Domain](#3-the-domain)
   - 3.1 [Lists](#31-lists)
     - 3.1.1 [Issue list](#311-issue-list)
     - 3.1.2 [The suggestion list](#312-the-suggestion-list)
   - 3.2 [Systemic Consensing](#32-systemic-consensing)
   - 3.3 [The Admin](#33-the-admin)
   - 3.4 [Economic Model and Treasury-Wallet](#34-economic-model-and-treasury-wallet)
     - 3.4.1 [Overview of Use-Cases for PNYX](#341-overview-of-use-cases-for-pnyx)
...
8. [Appendix](#8-appendix)
   - 8.1 [Prices and Rewards](#81-prices-and-rewards)
     - 8.1.1 [List of global constants and parameters and default values](#811-list-of-global-constants-and-parameters-and-default-values)
     - 8.1.2 [List of global input variables](#812-list-of-global-input-variables)
     - 8.1.3 [List of domain related input variables](#813-list-of-domain-related-input-variables)
     - 8.1.4 [Principals and explanation of the price calculations](#814-principals-and-explanation-of-the-price-calculations)
...
| Expiring date | none |
| Remove after inactivity | 360 days |
...
The user who created the domain has admin rights. This allows him to configure the domain for the respective task. The admin can set the options of the domain, but cannot delete content of others.

Optionally, the admin position can be set electable. In this case the admin rights are always held by the member with the highest rank on the member list. In private domains the admin can add new members to the member list, but this right can also be given to every member of the domain. The admin can also remove names from the member list. Since list entries can be removed if 67% of the domain members decide so, the admin can also be removed from the member list. In this case the highest ranking member on the list becomes admin.
...
In each domain, problem areas (issues) can be defined for which solution approaches (suggestions) can be introduced. Thanks to the implemented tokenomics, TR does not require moderation or hierarchies. The economics of permanent decision-making in TR follows three simple principles:
...
- **Exclude (delete) other members avatar names from the list of members**
...
settlement and governance chain. User-operated pruned nodes, mobile verifying
participants, local-first bill/discussion workflows and sandboxed domain apps
are delivered through reversible gates. The design introduces no Minima
dependency, second chain/token or bridge commitment. See the
...
The 'permission register' is emptied periodically, but all members (on the 'member-list') can onboard again automatically. This gives the opportunity to exclude members from the domain, by deleting their names from the member list.
...
|--------|-------------|-------|
| c_dom | Factor to calculate the initial coin-load needed to open a domain | 2 |
| c_put | Factor for put price (open issue, put suggestion) | 15 |
| stake | Minimum stake | 100,000 PNYX |
| supplymax | Fix max PNYX supply | **21 million** |
| c_earn | Factor for rewards (voting, stoning) | 1,000 |
| apy_dom | Initial interest rate for domain coin release per year | 0.25 |
| apy_node | Initial interest rate for node staking (staking reward) per year | 0.1 |
...
| f_release | Fraction of released coins; f_release = release / supplymax |
| T_dom | Approx time interval for domain payouts in years (1 sec = 1/(60\*60\*24\*365.25)) |
| T_node | Approx time interval for node payouts in years (1 sec = 1/(60\*60\*24\*365.25)) |
...
| payout | Sum of domain payouts during a time interval (T_dom) |
| n_user | Number of users in the domain |

**Formulas:**

Minimum price for opening a new domain (initial treasure):

> **eq.1:** `p_dom = fee * c_dom * c_earn`

Reward for a single evaluation (VoteToEarn, RateToEarn):

> **eq.2:** `p_rew = treasure / c_earn`

Price for putting content:

> **eq.3:** `p_put = min(p_rew * c_put, p_rew * n_user)`

Amount of interest for a domain per time interval:

> **eq.4:** `i_dom = min(treasure * apy_dom * T_dom * [1 - f_release], payout)`

Amount of node rewards per time interval (fees not included):

> **eq.5:** `i_node = stake * apy_node * T_node * [1 - f_release]`
...
Rewards earned over the VoteToEarn mechanism, are calculated as a fraction of the domain treasure (eq.2). If this fraction (1/c_earn) equals 0.001, the coin load in the treasure must be at least a thousand times the cost of a single transaction in order to compensate the cost to the user. If the value of the treasure is twice that high (c_dom = 2), the user gets back twice the fee he has to pay.

The following formula describes the relationship between treasure drainage (f_d) and the number of user payouts (n):
...
> `n = -log(f_d) / log(1/c_earn + 1)`

So if the initial value of the treasure is 2000 times the transaction fee and each reward equals 0.001\*treasure, the compensation point, where the payout equals the fee, will be reached after 694 payouts:
