...
services:
  ...
  truerepublic-indexer:
    ...
    environment:
      ...
      INDEXER_RPC_URL: http://truerepublic-node/rpc
...
The indexer uses the Compose-local nginx proxy path. Direct cross-container
access to node RPC port 26657 is intentionally unavailable because RPC binds
only to loopback in the node namespace.
