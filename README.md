...
<p align="center">
  <img src="https://img.shields.io/badge/tests-1104%20recovery--verified-orange" alt="Recovery-verified tests"/>
...
./scripts/go-packages.sh go test -race -cover -count=1 -timeout=600s    # 1,070 Go cases
...
**Known Limitations:** IBC staking/upgrade remains stubbed (PoD is used instead), a real ZKP prover/ceremony review is pending, and the migration-focused harness proves compatible binary replacement and fail-before-open rollback—not consensus-breaking state migration. Compromised-key incident/recovery runbook rehearsal remains pending even though rotation and revocation are implemented. Dashboard/application-panel runtime evidence, alert rules, objectives, escalation ownership, load/capacity/topology qualification, and independent operations review remain open. See [LIMITATIONS.md](docs/LIMITATIONS.md).
...
- 🟡 1,104 tests recovery-verified locally (1,070 Go + 26 Rust + 8 maintained-client), plus separately gated GH-56 rotation, GH-59 slashing, GH-60 inactive-validator genesis, and GH-61 legacy-authority migration process harnesses
...
> Historical test count: 577. The authoritative recovery-verified total is 1,104
> (1,070 Go + 26 Rust + 8 maintained-client), reproduced locally on the current branch.
